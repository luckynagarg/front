  ...
  return (
    <RoleShell role="PATIENT">
      <div className="mb-8">
        ...
        <h1 className="font-display mt-2 text-3xl font-semibold text-slate-950 sm:text-4xl">Medical Records</h1>
        <p className="mt-2 text-sm text-slate-500">Your complete medical history and documents.</p>
      </div>
      ...
      <div className="space-y-4">
        {reports.length === 0 ? (
          <div className="rounded-2xl border border-slate-200 bg-white p-12 text-center">
            ...
            <h2 className="font-display mt-4 text-xl font-semibold text-slate-950">No records</h2>
            <p className="mt-2 text-sm text-slate-500">No medical records uploaded yet.</p>
          </div>
        ) : (
          reports.map((report) => (
      ...
      {/* View Modal */}
      {viewing && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" role="dialog" aria-modal="true" aria-labelledby="med-title">
          ...
          <div className="relative w-full max-w-md rounded-2xl border border-slate-200 bg-white p-6 shadow-xl">
            <div className="flex items-start justify-between gap-4">
              <div>
                <p className="font-mono text-[10px] uppercase tracking-[0.2em] text-cyan-700">Medical Record</p>
                <h2 id="med-title" className="font-display mt-1 text-2xl font-semibold text-slate-950">{viewing.testName}</h2>
              </div>
