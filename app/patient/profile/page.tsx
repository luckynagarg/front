...
export default function PatientProfilePage() {
  ...
  const [patient, setPatient] = useState(getPatient("PAT-001"));
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    if (!session) { router.push("/login"); return; }
    setPatient(getPatient("PAT-001"));
    setLoading(false);
  }, [session, router]);
  ...
  return (
    <RoleShell role="PATIENT">
      <div className="mb-8">
        ...
        <p className="mt-2 text-sm text-slate-500">Your verified personal and healthcare information.</p>
      </div>
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-6 sm:p-8">
        <div className="flex flex-col gap-6 sm:flex-row sm:items-center">
          ...
          <div>
            <h2 className="font-display text-2xl font-semibold text-slate-950">{patient.name}</h2>
            <p className="mt-1 text-sm text-slate-500">Patient · HealthO member</p>
      ...
      <section className="mt-6 rounded-2xl border border-slate-200 bg-white p-6 sm:p-8">
        <SectionHeading icon={UserRound} title="Personal information" description="Your basic personal details." />
      ...
      <section className="mt-6 rounded-2xl border border-slate-200 bg-white p-6 sm:p-8">
        ...
        <div className="mt-7 grid gap-5 sm:grid-cols-2">
          <ReadOnlyField label="Email" value={patient.email} icon={Mail} />
          <ReadOnlyField label="Phone number" value={patient.phone} icon={Phone} />
        </div>
      </section>

      {/* Address */}
      <section className="mt-6 rounded-2xl border border-slate-200 bg-white p-6 sm:p-8">
        <SectionHeading icon={MapPin} title="Address" description="Your current residential address." />
        <div className="mt-7">
          <ReadOnlyField label="Address" value={patient.address} icon={MapPin} />
        </div>
      </section>

      {/* Account */}
      <section className="mt-6 rounded-2xl border border-slate-200 bg-white p-6 sm:p-8">
        <SectionHeading icon={CalendarDays} title="Account" description="Your HealthO account details." />
        <div className="mt-7 grid gap-5 sm:grid-cols-2">
          <ReadOnlyField label="Patient ID" value={patient.id} />
          <ReadOnlyField label="Account created" value={patient.createdAt} />
        </div>
      </section>
      ...
      <div className="mt-6 flex items-start gap-3 rounded-2xl border border-slate-200 bg-slate-50 p-5">
        ...
        <div>
          ...
          <p className="mt-1 text-sm leading-6 text-slate-500">
            Your verified profile information can only be updated by an authorized healthcare professional.
          </p>
        </div>
      </div>
    </RoleShell>
