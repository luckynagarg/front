...
import { getPharmacy } from "@/services/data";

export default function PharmacyProfilePage() {
  const router = useRouter();
  const session = getSession();
  const [pharmacy, setPharmacy] = useState(getPharmacy("PHARM-001"));
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    if (!session) { router.push("/login"); return; }
    setPharmacy(getPharmacy("PHARM-001"));
    setLoading(false);
  }, [session, router]);
  ...
  return (
    <RoleShell role="PHARMACIST">
      <div className="mb-8">
        ...
        <h1 className="font-display mt-2 text-3xl font-semibold text-slate-950 sm:text-4xl">Pharmacy Profile</h1>
        <p className="mt-2 text-sm text-slate-500">Your pharmacy details and settings.</p>
      </div>
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-6 sm:p-8">
        <div className="flex flex-col gap-6 sm:flex-row sm:items-center">
          <div className="flex h-24 w-24 items-center justify-center rounded-full bg-slate-950 text-3xl font-semibold text-white">
            {pharmacy.name.split(" ").map((n) => n[0]).slice(0, 2).join("").toUpperCase()}
          </div>
          <div>
            <h2 className="font-display text-2xl font-semibold text-slate-950">{pharmacy.name}</h2>
            <p className="mt-1 text-sm text-slate-500">Pharmacy · HealthO</p>
      ...
      <section className="mt-6 rounded-2xl border border-slate-200 bg-white p-6 sm:p-8">
        <SectionHeading icon={Pill} title="Pharmacy information" description="Your pharmacy details." />
        <div className="mt-7 grid gap-5 sm:grid-cols-2">
          <ReadOnlyField label="Pharmacy name" value={pharmacy.name} />
          <ReadOnlyField label="Pharmacist" value={pharmacy.pharmacistName} />
          <ReadOnlyField label="Address" value={pharmacy.address} icon={MapPin} />
          <ReadOnlyField label="License number" value={pharmacy.licenseNumber} />
        </div>
      </section>
      ...
      <section className="mt-6 rounded-2xl border border-slate-200 bg-white p-6 sm:p-8">
        ...
        <div className="mt-7 grid gap-5 sm:grid-cols-2">
          <ReadOnlyField label="Email" value={pharmacy.email} icon={Mail} />
          <ReadOnlyField label="Phone number" value={pharmacy.phone} icon={Phone} />
        </div>
      </section>
      ...
      <div className="mt-6 flex items-start gap-3 rounded-2xl border border-slate-200 bg-slate-50 p-5">
        ...
        <div>
          ...
          <p className="mt-1 text-sm leading-6 text-slate-500">
            Your verified pharmacy information can only be updated by authorized administrators.
          </p>
        </div>
      </div>
    </RoleShell>
