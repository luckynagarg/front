...
import { ClipboardList, Eye, CheckCircle2 } from "lucide-react";
...
export default function PharmacyOrdersPage() {
  ...
  useEffect(() => {
    ...
    setPrescriptions(getPrescriptions().filter((p) => p.status === "Dispensed" || p.status === "Processing"));
    setLoading(false);
  }, [session, router]);

  const handleComplete = (id: string) => {
    setPrescriptions(updatePrescriptionStatus(id, "Completed"));
    if (viewing?.id === id) {
  ...
  return (
    <RoleShell role="PHARMACIST">
      <div className="mb-8">
        <p className="font-mono text-[10px] uppercase tracking-[0.2em] text-cyan-700">Orders</p>
        <h1 className="font-display mt-2 text-3xl font-semibold text-slate-950 sm:text-4xl">Pharmacy Orders</h1>
        <p className="mt-2 text-sm text-slate-500">Track and manage pharmacy orders.</p>
      </div>
      ...
      <div className="space-y-4">
        {prescriptions.length === 0 ? (
          <div className="rounded-2xl border border-slate-200 bg-white p-12 text-center">
            ...
            <h2 className="font-display mt-4 text-xl font-semibold text-slate-950">No orders</h2>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" role="dialog" aria-modal="true" aria-labelledby="rx-title">
          ...
          <div className="relative w-full max-w-md rounded-2xl border border-slate-200 bg-white p-6 shadow-xl">
            <div className="flex items-start justify-between gap-4">
              <div>
                <p className="font-mono text-[10px] uppercase tracking-[0.2em] text-cyan-700">Order</p>
            ...
            {viewing.status !== "Completed" && (
              <button type="button" onClick={() => handleComplete(viewing.id)} className="mt-6 inline-flex h-11 w-full items-center justify-center gap-2 rounded-xl bg-emerald-600 text-sm font-semibold text-white transition hover:bg-emerald-700">
