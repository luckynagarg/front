...
import { Pill, Eye, Plus } from "lucide-react";
...
export default function DoctorPrescriptionsPage() {
  ...
  useEffect(() => {
    ...
    setPrescriptions(getPrescriptions().filter((p) => p.doctorId === "DOC-001"));
    setLoading(false);
  }, [session, router]);
    ...
    <RoleShell role="DOCTOR">
      <div className="mb-8 flex flex-col justify-between gap-5 sm:flex-row sm:items-end">
        <div>
          ...
          <p className="mt-2 text-sm text-slate-500">Manage your prescriptions.</p>
        </div>
      ...
      <div className="space-y-4">
        {prescriptions.length === 0 ? (
          <div className="rounded-2xl border border-slate-200 bg-white p-12 text-center">
            ...
            <p className="mt-2 text-sm text-slate-500">You haven&apos;t created any prescriptions yet.</p>
          </div>
        ) : (
