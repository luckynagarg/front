...
export default function DoctorPatientDetailPage() {
    ...
    return (
      <RoleShell role="DOCTOR">
        <div className="flex min-h-[60vh] flex-col items-center justify-center">
          ...
          <Link href="/doctor/patients" className="mt-4 inline-flex h-11 items-center rounded-xl bg-slate-950 px-5 text-sm font-semibold text-white hover:bg-slate-800">
            Back to patients
          </Link>
        </div>
      </RoleShell>
  ...
  const severeAllergies = allergies.filter((a) => a.severity === "Severe" && a.status === "Active");

  return (
    <RoleShell role="DOCTOR">
      <div className="mb-8">
        <Link href="/doctor/patients" className="mb-4 inline-flex items-center gap-2 text-sm text-slate-500 transition hover:text-slate-800">
          ← Back to patients
        </Link>
      ...
      {severeAllergies.length > 0 && (
        <div className="mb-6 rounded-2xl border border-red-200 bg-red-50 p-5">
          <div className="flex items-start gap-3">
            ...
            <div>
              <p className="text-sm font-semibold text-red-800">Severe Allergy Alert</p>
              ...
              {allergies.length === 0 ? (
                ...
                allergies.map((allergy) => (
                  <div key={allergy.id} className="flex items-center justify-between rounded-xl border border-slate-100 p-4">
                    ...
                    <span className={`rounded-full px-3 py-1 text-xs font-semibold ${allergy.severity === "Severe" ? "bg-red-50 text-red-700" : allergy.severity === "Moderate" ? "bg-amber-50 text-amber-700" : "bg-slate-100 text-slate-600"}`}>
                      {allergy.severity}
                    </span>
                  </div>
                ))
        ...
        <div className="min-w-0 space-y-6">
          ...
          <section className="rounded-2xl border border-slate-200 bg-white p-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="font-mono text-[10px] uppercase tracking-[0.2em] text-cyan-700">Actions</p>
                <h3 className="font-display mt-1 text-xl font-semibold text-slate-950">Clinical actions</h3>
              </div>
            </div>
            <div className="mt-5 space-y-3">
              <button type="button" className="flex w-full items-center justify-center gap-2 rounded-xl border border-slate-200 px-4 py-3 text-sm font-semibold text-slate-700 transition hover:bg-slate-50">
                <Pill size={16} />
                Add Prescription
              </button>
              <button type="button" className="flex w-full items-center justify-center gap-2 rounded-xl border border-slate-200 px-4 py-3 text-sm font-semibold text-slate-700 transition hover:bg-slate-50">
