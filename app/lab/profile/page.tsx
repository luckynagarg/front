...
import { getLab } from "@/services/data";

export default function LabProfilePage() {
  const router = useRouter();
  const session = getSession();
  const [lab, setLab] = useState(getLab("LAB-001"));
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    if (!session) { router.push("/login"); return; }
    setLab(getLab("LAB-001"));
    setLoading(false);
  }, [session, router]);
  ...
  return (
    <RoleShell role="LAB">
      <div className="mb-8">
        ...
        <h1 className="font-display mt-2 text-3xl font-semibold text-slate-950 sm:text-4xl">Lab Profile</h1>
        <p className="mt-2 text-sm text-slate-500">Your laboratory details and settings.</p>
      </div>
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-6 sm:p-8">
        <div className="flex flex-col gap-6 sm:flex-row sm:items-center">
          <div className="flex h-24 w-24 items-center justify-center rounded-full bg-slate-950 text-3xl font-semibold text-white">
            {lab.name.split(" ").map((n) => n[0]).slice(0, 2).join("").toUpperCase()}
          </div>
          <div>
            <h2 className="font-display text-2xl font-semibold text-slate-950">{lab.name}</h2>
            <p className="mt-1 text-sm text-slate-500">Laboratory · HealthO</p>
      ...
      <section className="mt-6 rounded-2xl border border-slate-200 bg-white p-6 sm:p-8">
        <SectionHeading icon={FlaskConical} title="Laboratory information" description="Your lab details." />
        <div className="mt-7 grid gap-5 sm:grid-cols-2">
          <ReadOnlyField label="Laboratory name" value={lab.name} />
          <ReadOnlyField label="Administrator" value={lab.adminName} />
          <ReadOnlyField label="Address" value={lab.address} icon={MapPin} />
          <ReadOnlyField label="Registration ID" value={lab.registrationId} />
        </div>
      </section>
      ...
      <section className="mt-6 rounded-2xl border border-slate-200 bg-white p-6 sm:p-8">
        ...
        <div className="mt-7 grid gap-5 sm:grid-cols-2">
          <ReadOnlyField label="Email" value={lab.email} icon={Mail} />
          <ReadOnlyField label="Phone number" value={lab.phone} icon={Phone} />
        </div>
      </section>
      ...
      <div className="mt-6 flex items-start gap-3 rounded-2xl border border-slate-200 bg-slate-50 p-5">
        ...
        <div>
          ...
          <p className="mt-1 text-sm leading-6 text-slate-500">
            Your verified laboratory information can only be updated by authorized administrators.
          </p>
        </div>
      </div>
    </RoleShell>
