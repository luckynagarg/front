...
export default function HospitalDashboard() {
  ...
  useEffect(() => {
    ...
    seedDataIfEmpty();
    setAppointments(getAppointments().slice(0, 5));
    setReports(getLabReports().slice(0, 5));
    setNotifications(getNotifications("HOSP-001").slice(0, 5));
    setLoading(false);
  }, [session]);
  ...
  return (
    <RoleShell role="HOSPITAL">
      <div className="mb-8">
        ...
        <h1 className="font-display mt-2 text-3xl font-semibold text-slate-950 sm:text-4xl">Hospital Dashboard</h1>
        <p className="mt-2 text-sm leading-6 text-slate-500">
          Manage patients, appointments and reports.
        </p>
      </div>

      <div className="mb-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <QuickAction href="/hospital/patients" label="Patients" icon={Users} />
        <QuickAction href="/hospital/appointments" label="Appointments" icon={CalendarDays} />
        <QuickAction href="/hospital/reports" label="Reports" icon={FileText} />
      </div>
          ...
          <section className="rounded-2xl border border-slate-200 bg-white p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <h3 className="font-display mt-2 text-xl font-semibold text-slate-950">Recent appointments</h3>
              </div>
              <Link href="/hospital/appointments" className="text-sm font-semibold text-cyan-700 hover:text-cyan-800">View all</Link>
            </div>
