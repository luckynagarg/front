...
export default function HospitalProfilePage() {
  ...
  const [hospital, setHospital] = useState(getHospital("HOSP-001"));
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    if (!session) { router.push("/login"); return; }
    setHospital(getHospital("HOSP-001"));
    setLoading(false);
  }, [session, router]);
  ...
  return (
    <RoleShell role="HOSPITAL">
      <div className="mb-8">
        ...
        <h1 className="font-display mt-2 text-3xl font-semibold text-slate-950 sm:text-4xl">Hospital Profile</h1>
        <p className="mt-2 text-sm text-slate-500">Your organization details and settings.</p>
      </div>
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-6 sm:p-8">
        <div className="flex flex-col gap-6 sm:flex-row sm:items-center">
          <div className="flex h-24 w-24 items-center justify-center rounded-full bg-slate-950 text-3xl font-semibold text-white">
            {hospital.name.split(" ").map((n) => n[0]).slice(0, 2).join("").toUpperCase()}
          </div>
          <div>
            <h2 className="font-display text-2xl font-semibold text-slate-950">{hospital.name}</h2>
            <p className="mt-1 text-sm text-slate-500">Hospital / Clinic · HealthO</p>
      ...
      <section className="mt-6 rounded-2xl border border-slate-200 bg-white p-6 sm:p-8">
        <SectionHeading icon={Building2} title="Organization information" description="Your hospital details." />
        <div className="mt-7 grid gap-5 sm:grid-cols-2">
          <ReadOnlyField label="Organization name" value={hospital.name} />
          <ReadOnlyField label="Administrator" value={hospital.adminName} />
          <ReadOnlyField label="Address" value={hospital.address} icon={MapPin} />
          <ReadOnlyField label="Registration ID" value={hospital.registrationId} />
        </div>
      </section>
      ...
      <section className="mt-6 rounded-2xl border border-slate-200 bg-white p-6 sm:p-8">
        ...
        <div className="mt-7 grid gap-5 sm:grid-cols-2">
          <ReadOnlyField label="Email" value={hospital.email} icon={Mail} />
          <ReadOnlyField label="Phone number" value={hospital.phone} icon={Phone} />
        </div>
      </section>
      ...
      <div className="mt-6 flex items-start gap-3 rounded-2xl border border-slate-200 bg-slate-50 p-5">
        ...
        <div>
          ...
          <p className="mt-1 text-sm leading-6 text-slate-500">
            Your verified organization information can only be updated by authorized administrators.
          </p>
        </div>
      </div>
    </RoleShell>
