...
export default function PatientSignupPage() {
  ...
  const handleSubmit = (event: React.FormEvent) => {
    ...
    signupAs(name.trim(), "PATIENT", email.trim());
  ...
  return (
    <SignupLayout
      ...
      description="Join HealthO to manage your health, appointments and records."
      backHref="/signup"
    ...
    >
      <form className="space-y-5" onSubmit={handleSubmit}>
        <div>
          ...
          <input id="name" type="text" value={name} onChange={(e) => setName(e.target.value)} placeholder="Your full name" className="h-12 w-full rounded-xl border border-slate-200 bg-white px-4 text-sm outline-none transition placeholder:text-slate-400 focus:border-cyan-600 focus:ring-4 focus:ring-cyan-600/10" />
        </div>
        <div>
          <label htmlFor="email" className="mb-2 block text-sm font-medium text-slate-700">Email</label>
          <input id="email" type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="you@example.com" className="h-12 w-full rounded-xl border border-slate-200 bg-white px-4 text-sm outline-none transition placeholder:text-slate-400 focus:border-cyan-600 focus:ring-4 focus:ring-cyan-600/10" />
        </div>
        <div>
